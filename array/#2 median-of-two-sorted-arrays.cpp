...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        if ((nums1.size() + nums2.size()) % 2 == 1)
            return findElementsInSortedArrays(arrays, (nums1.size() + nums2.size()) / 2 + 1);
        else
            return (findElementsInSortedArrays(arrays, (nums1.size() + nums2.size()) / 2) + 
                    findElementsInSortedArrays(arrays, (nums1.size() + nums2.size()) / 2 + 1)) / 2.0;
    }

private:
    int findElementsInSortedArrays(const vector<vector<int> *>& arrays, int target) {
    ...
    bool match(const vector<vector<int> *>& arrays, int num, int target) {
        ...
        for (const auto array : arrays) {
            if (!array->empty())
                dist += distance(upper_bound(array->cbegin(), array->cend(), num), array->cend());
        }
