...
private:
    int calculateArea(vector<int>& heights, int begin, int end) {
        // attention
        if (begin > end) {
            return 0;
        }
        ...
        return max(heights[minIdx] * (end - begin + 1),
                   max(calculateArea(heights, begin, minIdx - 1), calculateArea(heights, minIdx + 1, end)));
    }
};